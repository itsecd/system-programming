...
Главный поток создает два других потока и ждет завершения рабочего потока.
...
Программа аналогична предыдущей с той разницей, что вместо
ключевого слова `thread_local` явно используются функции работы с TLS.
...
`sigwaitinfo()` провалится с ошибкой `EINTR`.
...
но никакой реакции от него не поступает. Поскольку сигнал послан конкретному потоку, потоки 
3 и 4 также никак не реагируют на сигнал, хотя могли бы его принять.
