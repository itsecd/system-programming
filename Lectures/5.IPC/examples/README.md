...
Все программы, за исключением [4_semaphore](#4_semaphore) и [7_pipe_buffer](#7_pipe_buffer), решают одну и ту же задачу.
...
Структура сообщения и вспомогательные функции для работы с ним определены в файле [common.hpp](common.hpp)
...
Вариация предыдущего примера, в которой создается общая для двух процессов область памяти через
ыызов `mmap()` с флагами MAP_SHARED|MAP_ANONYMOUS.

Созданная таким образом область памяти не связана ни с каким файлом (а значит, не требует открытия/закрытия файлов).

## `3_shm`

Программа является модификацией примера [2_mmap](#2_mmap) с той лишь разницей, что вместо обычного файла 
...
на строках 44 и 48 процессы закрывают ненужные концы канала. Это необходимо, т.к. иначе не будет
работать детектирование "обрыва связи".
...
каждый процесс самостоятельно открывает канал в нужном режиме доступа.

При этом синхронизация процессов по открытии канала не требуется, т.к. `open()` приостанавливает вызывающий процесс до тех пор,
...
и максимальный размер сообщения, равный размеру структуры Message (строка 73).

По аналогии с файлами разделяемой памяти, имя очереди сообщений должно начинаться с /.
...
Программа демонстрирует эффекты неатомарной записи в канал.

Гарантируется, что запись в канал данных размером менее `PIPE_BUF` происходит атомарно. 
В противном случае записываемые данные могут быть разбиты на части, записываемые отдельно.
Если в канал одновременно пишут несколько потоков, записываемые части сообщений могут быть перемешаны.
В результате возникает риск прочтения "битых" данных.

Значение `PIPE_BUF` зависит от системы, в программе на С/С++ оно доступно в виде макроса из `limits.h`.
